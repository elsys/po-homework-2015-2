...
int exec_cmd(struct command_t*, char*, int);
int check_error1(char*);
int check_error2(char*);
int check_error3(char*);
int check_error4(char*, int*);
int check_error5(struct command_t*, int);
int check_valid_operators(char*, int);
int check_unary_operators(char*);
void arrange_command(struct command_t*, char*, int);
void find_result(struct command_t*, int);
...
int exec_cmd(struct command_t *cmd, char *input, int i){
...
int check_error1(char *input){
	...
	if(input[0] == ' ')
        i++;
	if(check_valid_operators(input, i-1) && input[i+1] == ' ' && i+1 < strlen(input))
        return 0;
	else if(input[i] == '+' && input[i+1] == '+' && input[i+2] == ' ' && i+2 < strlen(input))
        return 0;
	else{
...
int check_error2(char *input){
	...
	if(((input[i] != ' ' || input[i+2] != ' ' || i+3 >= strlen(input)) && check_valid_operators(input, i) ) || input[i+1] == '!')
        return 1;
	return 0;
}
...
void arrange_command(struct command_t *cmd, char *input, int i){
	int j = 0, k = 0, negative = 0;
	if(input[0] == ' ')
        k++;
	if(check_unary_operators(input)){
		cmd[i].operation = input[k];
		if(input[k+1] == cmd[i].operation)
            cmd[i].operation = '*';
		while(input[k+j] != ' ')
            j++;
		cmd[i].argument2 = 0;
...
int check_error5(struct command_t *cmd, int i){
	if((cmd[i].operation == '/' || cmd[i].operation == '%') && cmd[i].argument2 == 0)
        return 1;
	else
        return 0;
}

void find_result(struct command_t *cmd, int i){
	if(cmd[i].type == 1){
		if(cmd[i].operation == '!')
